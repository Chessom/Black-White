...
namespace std {
	...
	struct formatter<bw::othello::bitbrd_t<Size>, CharT> {
		bool linemark = false;
		bool right = true;
		...
		auto format(const bw::othello::bitbrd_t<Size>& brd, FormatContext& fc) {
			using namespace bw::othello;
			auto it = fc.out();
			for (int x = 0; x < Size; ++x) {
				for (int y = 0; y < Size; ++y) {
	...
	template<int Size, typename CharT>
	struct formatter<bw::othello::arrbrd_t<Size>, CharT> {
		bool linemark = false;
		bool right = true;
		constexpr auto parse(auto& context) {
			using namespace bw::othello;
		...
		template<typename FormatContext>
		auto format(const bw::othello::arrbrd_t<Size>& brd, FormatContext& fc) {
			using namespace bw::othello;
			auto it = fc.out();
			for (int x = 0; x < Size; ++x) {
				for (int y = 0; y < Size; ++y) {
	...
	struct formatter<bw::othello::dynamic_brd, CharT> {
		bool linemark = false;
		bool right = true;
		constexpr auto parse(auto& context) {
			using namespace bw::othello;
		...
		template<typename FormatContext>
		auto format(const bw::othello::dynamic_brd& brd, FormatContext& fc) {
			using namespace bw::othello;
			auto it = fc.out();
