...
namespace bw::othello {
	...
	public:
		...
		moves(const arrbrd_t<Size, PosType>& brd, const color& col) {
			update(brd, col);
		}
		moves(const dynamic_brd& brd, const color& col) {
			update(brd, col);
		}
		template<int Size>
		...
		template<int Size, std::integral PosType>
		void update(const arrbrd_t<Size, PosType>& brd, const color& col) {
			clear();
			using board = arrbrd_t<Size, PosType>;
			using drc_t = int;
		...
		void update(const dynamic_brd& brd, const color& col) {
			clear();
			using board = dynamic_brd;
			using drc_t = int;
			using namespace directions;
			color opcol = col ^ 1;
